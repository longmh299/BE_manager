...
/**
 * ✅ IMPORTANT FIX (TS2322):
 * Schema netTotal của bạn là Decimal NOT NULL => không được filter "netTotal: null".
...
export async function getReceivablesReport(params: { asOf?: string; includeRows?: boolean }) {
  ...
  // allocations theo invoiceId + kind (signed sum)
  ...
  for (const inv of invoices as any[]) {
    ...
    const paidTotal = D(inv.paidAmount); // paid NORMAL net
