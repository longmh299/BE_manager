...
r.get('/', async (req, res, next)=> {
  try {
    ...
    res.json({ ok: true, total, page: Number(page), pageSize: Number(pageSize), data });
  } catch (e) { next(e); }
});
...
r.post('/', requireRole('accountant','admin'), async (req, res, next)=> {
  try {
    const { type, refNo, note } = req.body as { type: MovementType, refNo?: string, note?: string };
    // createDraft của bạn hiện đang nhận (type, payload) hoặc (type, refNo, note) tuỳ bản
    // Giữ đúng theo service hiện tại của bạn. Nếu bạn dùng bản (type, refNo, note) thì dòng dưới ok.
...
r.post('/:id/lines', requireRole('accountant','admin'), async (req,res,next)=>{
  try {
    ...
    const data = await addLine(movementId, { itemId, fromLocationId, toLocationId, qty, note });
    res.json({ ok: true, data });
  } catch (e) { next(e); }
});
