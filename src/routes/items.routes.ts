...
import { listItems, createItem, updateItem, removeItem } from '../services/items.service';
 ...
 */
r.post('/import', requireRole('accountant', 'admin'), upload.single('file'), async (req, res, next) => {
  try {
    if (!req.file) throw new Error('Missing file field "file"');
    const result = await importItemsFromBuffer(req.file.buffer);
